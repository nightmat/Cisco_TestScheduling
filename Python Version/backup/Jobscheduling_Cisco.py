...
class CiscoJobSchedulingExecute():
    ...
    def getValues_1(self, MaxIterations=500):
        s = [OpOEA()]
        # s_name = ["(1+1)EA"] # call the (1+1) Evolutionary algorithm
        ...
        for sea in range(len(s)):
            for k in range(self.loopNum):
                ...
                p.setjobsMin(self.jobsMin)   # set the minimum of number of output jobs, which is 1
                p.setjobsMax(self.jobsMax)  # set the maximum of number of output jobs, which is the length of input test job list
...
# A job means a test case which including a set of attributes.
...
#2) weight is used to represent the weight of a test resource based on the available number of test resources in the pool and associated number of features; 
...
#4) subnetcost shows the cost for setting up correct subnet and the current value is simulated from 1 mins to 10 mins;
...
#give a template of the problem
    ...
    def getFitness(self, v):
        ...
        while index < v[0]:
            ...
            if gap_of_time <= 60:
                ...
                if tag_of_record:
                    selectedjobs.append(self.jobs[jobId])
...
#the framework of the search algorithm
